...
def import_artists():
    ...
    with open(data_filepath, "r", encoding='utf8') as f:
        ...
        for row in reader:
            ...
            new_artist, created = models.Artist.objects.get_or_create(name=in_artist)
...
def import_showsongs():
    ...
    with open(data_filepath, "r", encoding='utf8') as f:
        ...
        for row in reader:
            ...
            new_perf = models.ShowSong(show=show, song=song, set=set, track=row[4], segue=row[5], notes=row[6], guest=row[8])

            new_perf.save()

            results["created"] += 1
    print("Finished - Created {} ShowSongs".format(results["created"]))
